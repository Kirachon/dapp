...
dotenv.config({ path: process.env.ENV_FILE || (process.env.NODE_ENV === 'staging' ? '.env.staging' : '.env') });
...
import { plugin as supertokensPlugin, errorHandler, wrapRequest, wrapResponse, middleware as stFastifyMiddleware } from 'supertokens-node/framework/fastify';
import { photoRoutes } from './routes/photos';
import { SocketIOService, socketService } from './services/socketio';
import { contentService } from './services/content';
import { logger, correlationIdMiddleware, requestLoggingMiddleware, responseLoggingHook } from './lib/logger';
...
function checkSignupRateLimit(ip: string | undefined): { allowed: boolean; retryAfterSec?: number } {
...
async function start() {
  ...
  const setupMockAuth = (fastify: any) => {
    ...
    if (isProduction) {
      console.warn('⚠️  Mock authentication explicitly enabled in production via ALLOW_MOCK_AUTH=true');
    } else {
    ...
    fastify.post('/auth/signup', async (request: any, reply: any) => {
      try {
        ...
        if (!email || !password) {
          return reply.status(400).send({ status: 'GENERAL_ERROR', message: 'Email and password required' });
        }
...
const getCorsConfig = () => {
  ...
  if (isProduction) {
    if (allowedOrigins.length === 0) {
      console.error('🚨 SECURITY WARNING: No ALLOWED_ORIGINS set in production. CORS will deny all origins.');
    ...
    console.log(`🧪 Development CORS: Allowing origins: ${Array.isArray(origin) ? origin.join(', ') : origin}`);
  }
  ...
  const getCSPConfig = () => {
    ...
    if (isProduction) {
      ...
      return {
        ...
        connectSrc: ["'self'", "wss:", "ws:", ...allowedOrigins.map(origin => origin.replace('http', 'ws'))],
        fontSrc: ["'self'", "https:"],
        mediaSrc: ["'self'"],
        workerSrc: ["'self'"],
        manifestSrc: ["'self'"]
  ...
  const resolvers = {
    Query: {
      ...
      activationData: async (_: unknown, __: unknown, ctx: any) => {
        ...
        const currentWelcomeStep = Math.min(6, 1 + (isVerified ? 1 : 0) + (hasBio ? 1 : 0) + (hasPhoto ? 1 : 0));

        const activationScoreBase = (isVerified ? 40 : 10) + (hasPhoto ? 30 : 0) + (hasBio ? 20 : 0) + 10;
      ...
      onboardingData: async (_: unknown, __: unknown, ctx: any) => {
        ...
        const steps = {
          1: { name: 'registration', completed: true, completedAt: user.createdAt.toISOString() },
          2: { name: 'emailVerification', completed: !!isVerified, completedAt: isVerified ? (user.lastActiveAt ? user.lastActiveAt.toISOString() : user.createdAt.toISOString()) : null },
          3: { name: 'profileSetup', completed: hasBio, completedAt: hasBio ? (user.lastActiveAt ? user.lastActiveAt.toISOString() : user.createdAt.toISOString()) : null },
          4: { name: 'avatarUpload', completed: hasPhoto, completedAt: hasPhoto ? (user.lastActiveAt ? user.lastActiveAt.toISOString() : user.createdAt.toISOString()) : null },
          5: { name: 'guidedTour', completed: false, completedAt: null },
          6: { name: 'activation', completed: isVerified && hasBio && hasPhoto, completedAt: null },
        ...
        const milestones = {
          ...
          profileCompleted: welcomeSequenceCompleted ? (user.lastActiveAt ? user.lastActiveAt.toISOString() : null) : null,
          firstGoalSet: null,
          weeklyActive: true,
        };
      ...
      myProfile: async (_: unknown, __: unknown, ctx: any) => {
        ...
        if (process.env.NODE_ENV !== 'production') {
          console.log(`[${new Date().toISOString()}] 🔎 myProfile resolver`, { userId: ctx.user.id, hasProfile: !!p, name: p?.name, age: p?.age });
        }
        return p;
      },
      // Preferences
      ...
      adminUsers: requireAdmin(async (_: unknown, args: {
        limit?: number;
        offset?: number;
        status?: string;
        search?: string
      ...
      adminModeration: requireAdmin(async (_: unknown, args: {
        limit?: number;
        offset?: number;
        status?: string;
        priority?: string
      ...
      adminReports: requireAdmin(async (_: unknown, args: {
        limit?: number;
        offset?: number;
        status?: string
      }) => {
    ...
    Mutation: {
      signUp: async (_: unknown, args: { email: string; password: string; acceptTerms: boolean }, ctx: { request: any, reply: any }) => {
        try {
      ...
      resetPassword: async (_: unknown, { email }: { email: string }, ctx: { request: any, reply: any }) => {
        try {
          const req = wrapRequest(ctx.request);
          const res = wrapResponse(ctx.reply);
          await EmailPassword.createResetPasswordToken('public', email, undefined as any, { request: req, response: res } as any);
          return true;
        } catch (error) {
      ...
      resendVerificationEmail: async (_: unknown, __: unknown, ctx: { request: any, reply: any, user?: any }) => {
        try {
          ...
          try {
            const isVerified = await (EmailVerification as any).isEmailVerified('public', ctx.user.id, ctx.user.email, undefined, { request: req, response: res } as any);
            if (isVerified) return true;
          } catch {}

          // Create a new verification token
          const tokenRes = await (EmailVerification as any).createEmailVerificationToken('public', ctx.user.id, ctx.user.email, undefined, { request: req, response: res } as any);
          ...
          const payload = {
            ...
            content: [{ type: 'text/html', value: `<p>Please verify your email by clicking <a href="${link}">this link</a>.</p>` }],
          } as any;
      ...
      upsertMyProfile: async (_: unknown, { input }: { input: any }, ctx: any) => {
        ...
        if (!ctx?.user?.id) {
          console.error('❌ upsertMyProfile: No authenticated user in context');
        ...
        try {
          ...
          console.error(`❌ Database error in upsertMyProfile for user ${ctx.user.id}:`, err?.message || dbError);

          // For database connection issues, return a mock profile to allow the flow to continue
          if (typeof err?.message === 'string' && (err.message.includes('Authentication failed') || err.message.includes('Connection'))) {
      ...
      upsertMyPreferences: async (_: unknown, { input }: { input: any }, ctx: any) => {
        ...
        try {
          ...
          console.error(`❌ Database error in upsertMyPreferences for user ${ctx.user.id}:`, (dbError as any)?.message || dbError);
      ...
      changeMyPassword: async (_: unknown, { currentPassword, newPassword }: { currentPassword: string; newPassword: string }, ctx: any) => {
        try {
          if (!ctx?.user?.id || !ctx?.user?.email) throw new Error('Unauthenticated');
          if (typeof newPassword !== 'string' || newPassword.length < 8) throw new Error('Invalid password');
          const req = wrapRequest(ctx.request); const res = wrapResponse(ctx.reply);
          // Verify current password
          // @ts-ignore
          const signInRes = await (EmailPassword as any).signIn('public', ctx.user.email, currentPassword, { request: req, response: res });
          if (signInRes.status !== 'OK') return false;
          // @ts-ignore
          const upd = await (EmailPassword as any).updateEmailOrPassword('public', ctx.user.id, undefined, newPassword, { request: req, response: res });
      ...
      changeMyEmail: async (_: unknown, { newEmail }: { newEmail: string }, ctx: any) => {
        try {
          ...
          const req = wrapRequest(ctx.request); const res = wrapResponse(ctx.reply);
          // @ts-ignore
          const upd = await (EmailPassword as any).updateEmailOrPassword('public', ctx.user.id, email, undefined, { request: req, response: res });
          if (upd.status !== 'OK') return false;
          // Create verification token for the new email and send
          try {
            // @ts-ignore
            const tokenRes = await (EmailVerification as any).createEmailVerificationToken('public', ctx.user.id, email, undefined, { request: req, response: res });
            if (tokenRes.status === 'OK' && tokenRes.token) {
              const webDomain = process.env.WEB_DOMAIN || 'http://localhost:3000';
              const link = `${webDomain}/auth/verify-email?token=${tokenRes.token}`;
              const SENDGRID_API_KEY = process.env.SENDGRID_API_KEY || '';
              ...
              const FROM_NAME = process.env.SENDGRID_FROM_NAME || 'LoveConnect';
              if (SENDGRID_API_KEY) {
                const payload = { personalizations: [{ to: [{ email }] }], from: { email: FROM_EMAIL, name: FROM_NAME }, subject: 'Verify your new email address', content: [{ type: 'text/html', value: `<p>Verify your new email by clicking <a href="${link}">this link</a>.</p>` }] } as any;
                await fetch('https://api.sendgrid.com/v3/mail/send', { method: 'POST', headers: { 'Authorization': `Bearer ${SENDGRID_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
              } else {
                console.warn('SENDGRID_API_KEY not set; cannot send change-email verification');
              }
            }
          } catch (e) { console.error('changeMyEmail verification send error', e); }
          return true;
        } catch (e) {
      ...
      deleteMyAccount: async (_: unknown, __: unknown, ctx: any) => {
        try {
          ...
          try {
            ...
            await prisma.conversation.deleteMany({ where: { match: { OR: [{ userIdA: userId }, { userIdB: userId }] } } as any });
            await prisma.match.deleteMany({ where: { OR: [{ userIdA: userId }, { userIdB: userId }] } });
            await prisma.location.deleteMany({ where: { userId } });
            await prisma.preferences.deleteMany({ where: { userId } });
            await prisma.profile.deleteMany({ where: { userId } });
            await prisma.user.deleteMany({ where: { id: userId } });
          } catch (dbErr) { console.error('DB cleanup error during deleteMyAccount', dbErr); }
          try {
            // @ts-ignore
            if ((supertokens as any).deleteUser) { await (supertokens as any).deleteUser(userId); }
          } catch (stErr) { console.error('SuperTokens deleteUser error', stErr); }
          return true;
        } catch (e) {
      ...
      sendWelcomeEmail: async (_: unknown, __: unknown, ctx: any) => {
        try {
          ...
          const payload = { personalizations: [{ to: [{ email }] }], from: { email: FROM_EMAIL, name: FROM_NAME }, subject: 'Welcome to LoveConnect', content: [{ type: 'text/html', value: `<h2>Welcome!</h2><p>Thanks for joining LoveConnect. You’re all set.</p>` }] } as any;
      ...
      // Admin mutations
      adminBanUser: requireAdmin(async (_: unknown, { userId, reason }: { userId: string; reason: string }) => {
      ...
      adminModerationAction: requireAdmin(async (_: unknown, {
        itemId,
        action,
        reason
      }: {
      ...
      adminResolveReport: requireAdmin(async (_: unknown, {
        reportId,
        action,
        reason
      }: {
      ...
      updateFeatureFlag: requireAdmin(async (_: unknown, {
        key,
        enabled,
        rolloutPercentage
      }: {
      ...
      updateEmailTemplate: requireAdmin(async (_: unknown, {
        key,
        subject,
        htmlContent,
        textContent,
      ...
      updateAppConfig: requireAdmin(async (_: unknown, {
        key,
        value,
        description
      }: {
      ...
      updateContentPage: requireAdmin(async (_: unknown, {
        slug,
        title,
        content,
        metaDescription,
  ...
  await server.register(async function (fastify) {
    ...
    fastify.addHook('preHandler', async (request, reply) => {
      try {
        ...
        const s = await Session.getSession(request as any, reply as any, { sessionRequired: false });
        ...
        if (s) {
          ...
          try {
            ...
            console.warn('⚠️ Database sync failed (continuing with session-only auth):', (userError as any)?.message || userError);
            ...
            try {
              ...
              console.warn('⚠️ Failed to get SuperTokens user data:', (stError as any)?.message || stError);
            }
          }
          ...
          console.log(`✅ [${new Date().toISOString()}] preHandler: session found userId=${userId}, email=${email}, path=${(request as any).url}`);
          (request as any).user = { id: userId, email, roles };
        } else {
          console.log(`🔍 [${new Date().toISOString()}] preHandler: no session for ${(request as any).method} ${(request as any).url}`);
      ...
      handler: fastifyApolloHandler(apollo, {
        context: async (request: any, reply: any) => {
          try {
            ...
            logger.error('GraphQL context creation error', {
